...
~~~mermaid
flowchart LR
    A[Original schedule] --> B[Compute candidate replacement]
    B --> C{Ascending pair exists?}
    C -- yes --> D[Replace selected losing position with E]
    C -- no --> E[Try replacing each non-E position with E]
    D --> F[Recalculate salary]
    E --> F
    F --> G[Print maximum salary]
~~~
